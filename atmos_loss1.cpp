...
int main() {
    ...
    x,X_loss,v_esc,Mt,mu,k,T,G,g,p_base,A,V,Rd,N,r_gi,sum;
    ...
    else if (m_imp <= m_min){ // if mass of impactor is less than minimum mass required to eject any atmosphere
        m_eject = 0;    
    }
        
    
    if (r > r_gi && answer == '1'){ // Adiabatic atmosphere {
        X_loss = .4*x + (1.8*pow(x,2)) -1.2*pow(x,3); // global mass loss for an adiabatic atmosphere
        std::cout<<"Mass lost is:"<<X_loss+m_eject<<endl;   
    }
    if (r>r_gi && answer == '2'){ // Isothermal atmosphere
